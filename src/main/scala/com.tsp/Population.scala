...
case class Population(value: Vector[Chromosome]) extends AnyVal {
  def fittest(costMatrix: CostMatrix, source: Node): Unit = {
  ...
  def evolve(costMatrix: CostMatrix, source: Node): Population = {
    val size = value.size
  ...
  final def bulkEvolve(costMatrix: CostMatrix, source: Node)(generations: Int): Population =
    if (0 < generations)
      evolve(costMatrix, source).bulkEvolve(costMatrix, source)(generations - 1)
    else
      evolve(costMatrix, source)

  private def pseudoRandomSelect(costMatrix: CostMatrix, source: Node): Chromosome = {
  ...
  private def pseudoRandomSelectTwoUnique(costMatrix: CostMatrix, source: Node): (Chromosome, Chromosome) = {
  ...
  private def pseudoRandomSelectTwoNonUnique(costMatrix: CostMatrix, source: Node): (Chromosome, Chromosome) =
    (pseudoRandomSelect(costMatrix, source), pseudoRandomSelect(costMatrix, source))

  private def byFitnessCumulative(costMatrix: CostMatrix, source: Node): Vector[(Chromosome, Double, Double)] = {
  ...
  private def byFitnessNormalized(costMatrix: CostMatrix, source: Node): Vector[(Chromosome, Double)] = {
    val sorted = byFitness(costMatrix, source)
  ...
  private def byFitness(costMatrix: CostMatrix, source: Node): Vector[(Chromosome, Double)] = value.map(chromosome =>
...
object Population {
  def random(costMatrix: CostMatrix, source: Node): Population = Population((0 until Config.GA.populationSize).toVector
    .map(_ => Chromosome.random(costMatrix, source)))

  private def fittestOf(population: Vector[Chromosome])(costMatrix: CostMatrix, source: Node): Population = {
    Population(sortedWithCost(population)(costMatrix, source).take(populationSize).map {
      case (chromosome, _) => chromosome
    })
  }

  private def sortedWithCost(population: Vector[Chromosome])(costMatrix: CostMatrix, source: Node) = {
